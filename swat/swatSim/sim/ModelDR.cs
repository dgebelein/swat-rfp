...
namespace swatSim
{
	public class ModelDR : ModelBase
	{
		...
		public override SimParamData GetDefaultParams()
		{
			...
			p.InitItem("dr.SummerSoilRel", 0.5, typeof(double), 0.0, 1.0, "Gewichtung Boden-/Lufttemp ab 1.6.", 1); //0= 0%Boden, 100%Luft; 0.5= (Boden +Luft)/2; 1= 100% Boden
			...
			p.InitItem("dr.TransWiPupa", 6.1, typeof(double), 5.0, 50.0, "Faktor Transition Überwinterungsstadium->Adult", 3);
			...
			p.InitItem("dr.FertCluster", 1, typeof(int), 1.0, 10.0, "Fertilität: Eiablage-Cluster", 3);
		...
		{
			return (double)_workingParams.GetValue("dr.StartAge");
		}
		...
		{
			get { return (int)_workingParams.GetValue("dr.FertCluster"); }
		}
