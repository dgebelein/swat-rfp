...
namespace SwatPresentations
{
	public class ChartGrid
	{
		...
		public void SetScaling(TtpTimeRange timeRange,
									  Double scaleYLMin, Double scaleYLMax, Double firstYLTick, Double deltaYLTicks,
									  Double scaleYRMin, Double scaleYRMax, Double firstYRTick, Double deltaYRTicks)
		{
			_timeRange = timeRange;
			SetXScaling();// scaleXMin, scaleXMax, firstXTick, deltaXTicks);
			SetYScaling(TtpEnAxis.Left, scaleYLMin, scaleYLMax, firstYLTick, deltaYLTicks);
			SetYScaling(TtpEnAxis.Right, scaleYRMin, scaleYRMax, firstYRTick, deltaYRTicks);

		}

		public void SetScaling(Double scaleXMin, Double scaleXMax, Double firstXTick, Double deltaXTicks,
							  Double scaleYLMin, Double scaleYLMax, Double firstYLTick, Double deltaYLTicks,
							  Double scaleYRMin, Double scaleYRMax, Double firstYRTick, Double deltaYRTicks)
		{
		...
		public void SetXScaling(Double scaleXMin, Double scaleXMax, Double firstXTick, Double deltaXTicks)
		{
			...
			_firstXTick = firstXTick;
			_deltaXTicks = Math.Abs(deltaXTicks);
		}

		public void SetYScaling(TtpEnAxis axis, Double scaleYMin, Double scaleYMax, Double firstYTick, Double deltaYTicks)
		{
		...
		public void Draw(bool drawXYGrid= false)
		{
			Rectangle rect = new Rectangle
			{
				Width = _canvas.Width,
		...
		private void AddVertXLines()
		{
			...
			while (xTick < _scaleXMax)
			{
				...
				tickLine.StrokeDashArray = new DoubleCollection(new double[2] { 1, 2 });

				tickLine.X1 = NormalizeX(xTick);
				tickLine.Y1 = 0;
				tickLine.X2 = NormalizeX(xTick);
				tickLine.Y2 = _canvas.Height;
				_canvas.Children.Add(tickLine);
