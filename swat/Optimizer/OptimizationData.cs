...
namespace swat.optimizer
{
	...
	public class OptimizationData
	{
		...
		public OptimizationData(WorkspaceData workspace,ProgressChangedEventHandler progressChangedMethod, RunWorkerCompletedEventHandler progressCompletedMethod)
		{
			...
			_randomNumbers = new double[10000000];
		...
		private void ExecOptimizationLoops()
		{
			...
			_optimizer.ReportProgress(0, _graphData);
			OptimizeParameterset(VaryParameters(0, 0.1));
		...
		private double[] VaryParameters(int varyIndex, double amount)
		{
		...
		private ModelBase CreateOptimizationModel(SimParamData optParams)
		{
			...
			switch (_workspace.CurrentFlyType)
			{
				case FlyType.DR: model = new ModelDR("",_workspace.WeatherData, population, _workspace.CurrentWorkingParameters); break;
				case FlyType.PR: model = new ModelPR("",_workspace.WeatherData, population, _workspace.CurrentWorkingParameters); break;
				case FlyType.DA: model = new ModelDA("", _workspace.WeatherData, population, _workspace.CurrentWorkingParameters); break;
		...
		double GetModelParamEvalValue(double[] solverFactors)
		{
			...
			{
				model.RunSimulation();
				_quantor = Quantor.CreateNew(model.Population, _workspace.CurrentMonitoringData, EvalMethod.AbsDiff, false);  
