...
namespace SwopReview
{
	class Reporter
	{
		...
		void AddOptParameters()
		{
			...
			{
				string desc = _data.DefaultParameters.ParamDict.ContainsKey(p) ?
		...
		string GetStepParamString(int step)
		{
			...
			foreach(string param in _data.OptParameters)
			{
				...
				//sb.Append($"{param} = {_data.OptParamValues[step,p].ToString("0.0###",CultureInfo.InvariantCulture)}");
				p++;
			}
