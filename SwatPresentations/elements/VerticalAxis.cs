...
namespace SwatPresentations
{
	public class VerticalAxis
	{
		...
		{
			DrawAxis(null, showLabels, false);
		}
		public void Draw(bool showTicks, bool showLabels = true)
		{
			DrawAxis(null, showLabels, showTicks);
		}

		public void Draw(String[] labels)
		{
			DrawAxis(labels, true, false);
		}

		private void DrawAxis(String[] labels, bool showLabels, bool showTicks)
		{
			...
			while (yTick < (_scaleMax +  _deltaTicks/2.0)) // wg. Rundungsfehlern
			{
				...
				if (labels == null)
				{
					tb.Text = yTick.ToString(labelFmt, CultureInfo.InvariantCulture);
				}
				else
				{
