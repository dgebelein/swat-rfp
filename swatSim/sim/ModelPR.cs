...
namespace swatSim
{
	public class ModelPR : ModelBase
	{
		...
		public ModelPR(string name, WeatherData weather, PopulationData population, SimParamData explicitParams = null, SimParamData locationParams = null) 
			: base(name, weather, population, explicitParams, locationParams)
		{
		...
		public override SimParamData GetDefaultParams()
		{
			...
			p.InitItem("pr.MaxGen", 5, typeof(int), 4, 7, "maximale Generationenzahl", 3);
			...
			p.InitItem("pr.SummerSoilRel", 0.9, typeof(double), 0.0, 1.0, "Mischungsverh. Boden-/Lufttemp ab 1.6.", 1); //0= 0%Boden, 100%Luft; 0.5= (Boden +Luft)/2; 1= 100% Boden
			...
			p.InitItem("pr.TransWiPupa", 0.14, typeof(double), 0.0, 0.2, "Faktor Transition Überwinterungsstadium->Adult", 3);
			p.InitItem("pr.TransEgg", 0.14, typeof(double), 0.0, 0.2, "Faktor Transition Ei->Larve", 3);
			p.InitItem("pr.TransLarva", 0.14, typeof(double), 0.0, 0.2, "Faktor Transition Larve->Puppe", 3);
			p.InitItem("pr.TransPupa", 0.14, typeof(double), 0.0, 0.2, "Faktor Transition Puppe->Adult", 3);
			p.InitItem("pr.TransFly", 0.14, typeof(double), 0.0, 0.2, "Faktor Transition Adult->Tod", 3);
			...
			p.InitItem("pr.FertPost", 0.7855, typeof(double), 0.5, 1.0, "Fertilität: Post-Oviposition", 3);
		...
		{
			MaxGenerations = (int)_workingParams.GetValue("pr.MaxGen");
		}
